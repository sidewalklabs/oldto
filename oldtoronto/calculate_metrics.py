...
def tally_stats(truth_features, computed_features):
    ...
    out.writerow([
        'id', 'title',
        'computed date', 'true date', 'date match', 'date reason',
        'computed location', 'true location', 'location match', 'location reason',
    ...
    for feature in computed_features:
        ...
        if computed_coords:
            ...
            search_term = ''
        out.writerow([str(x) for x in (
            id_, props['title'],
            computed_date, true_date, date_match, date_reason,
            _coord_to_str(computed_coords), _coord_to_str(true_coords),
            geocode_match, geocode_reason,
            search_term,
...
''' % (
        num_dated_correct, num_datable, 100.0 * num_dated_correct / num_datable,
        num_geocoded_correct, num_geocodable, 100.0 * num_geocoded_correct / num_geocodable,
        num_geocoded_incorrect, num_geocoded, 100.0 * num_geocoded_incorrect / num_geocoded,
    ))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--computed_data', type=str,
